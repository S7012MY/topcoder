...
struct Pillars {
	double getExpectedLength(int w, int x, int y) {
		double rez=0;
		/*for(int i=1; i<=x; ++i) for(int j=1; j<=y; ++j) rez+=sqrt(w*w+abs(j-i)*abs(j-i));*/
		for(int i=0; i<=max(x,y); ++i) {
			int jos=i,sus=min(y,x+i);
			if(sus-jos+1>=0) pos[i]=sus-jos+1;
			jos=i,sus=min(x,y+i);
			if(sus-jos+1>=0) pos[i]+=sus-jos+1;
		}
		for(int i=0; i<=max(x,y); ++i) rez+=pos[i]*sqrt(w*w+i*i);
		rez/=(1.0*x*y);
		return rez;
	}
};
