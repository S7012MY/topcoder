...
struct CasketOfStar {
	int memo(int a, int b) {
		if(bst[a][b]!=MULT) return bst[a][b];
		int ret=MULT;
		if(a+1==b) {
			bst[a][b]=0;
			return 0;
		}
		for(int i=a+1; i<b; ++i) ret=max(ret,memo(a,i)+memo(i,b));
		ret+=w[a]*w[b];
		bst[a][b]=ret;
		return ret;
	}
	int maxEnergy(vector <int> weight) {
		w=weight;
		for(int i=0; i<weight.size(); ++i) for(int j=0; j<weight.size(); ++j) bst[i][j]=MULT;
		return memo(0,weight.size()-1);
	}
};
