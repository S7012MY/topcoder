...
struct MatrixGame {
	vector <string> getMinimal(vector <string> matrix) {
		vector <string> rez=matrix;
		vector<int> p;
		for(int i=0; i<matrix[0].size(); ++i) p.push_back(i);
		do {
			vector<string> act;
			for(int i=0; i<matrix.size(); ++i) act.push_back(matrix[0]);
			for(int j=0; j<matrix.size(); ++j) for(int i=0; i<matrix[j].size(); ++i)
				act[j][p[i]]=matrix[j][i];
