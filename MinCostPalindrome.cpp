...
struct MinCostPalindrome {
	int getMinimum(string s, int oCost, int xCost) {
		int rez=0,n=s.size(),cmi,cma;
		char mi,ma;
		cmi=min(oCost,xCost);cma=max(oCost,xCost);
		if(oCost>xCost) mi='x',ma='o';
		else mi='o',ma='x';
		for(int i=0; i<n/2; ++i)
			if(s[i]=='?' && s[n-i-1]=='?') {
				s[i]=s[n-i-1]=mi;
		...
		for(int i=0; i<n; ++i) {
			if(s[i]=='?') {
				if(s[n-i-1]=='x') s[i]='x',rez+=xCost;
				else s[i]='o',rez+=oCost;
				//cout<<s<<' ';
			}else if(s[i]!=s[n-i-1] && s[n-i-1]!='?') return -1;
		}
		return rez;
	}
};
