...
struct EllysDirectoryListing {
	vector <string> getFiles(vector <string> files) {
		vector <string> rez=files;
		int n=files.size();
		for(int i=0; i<n; ++i) if(files[i].compare("..")==0) files[i]=".";
		if(files[n-1]=="." && files[n-2]==".") return rez;
		for(int i=0; i<n; ++i) if(files[i]==".") {
